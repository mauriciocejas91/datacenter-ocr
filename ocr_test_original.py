...
def preprocess_image(image):
    ...
    # Eliminar ruido mediante el uso de un filtro de mediana
    denoised = cv2.medianBlur(binary, 3)
    
    # Convertir de nuevo a formato PIL para continuar con el OCR
    enhanced_image = Image.fromarray(denoised)
    
    # Ajustar contraste para mejorar la visibilidad de los textos
    enhancer = ImageEnhance.Contrast(enhanced_image)
    enhanced_image = enhancer.enhance(2.0)  # Factor de contraste ajustable
    
    return enhanced_image

# Función para extraer texto de imágenes PNG
...
def extract_text_from_pdf(pdf_path):
    ...
    for page_num, page in enumerate(pages):
        ...
        # Extraer texto usando Tesseract
        text = pytesseract.image_to_string(enhanced_page)
